...
video = cv2.VideoWriter('traffic_counter.mp4',cv2.VideoWriter_fourcc(*'MP4V'), fps, (height2, width2), 1)
...
while True:
    ...
    if ret:  # if there is a frame continue with code
       ...
       # empty list to later check which centroid indices were added to dataframe
        ...
        cv2.rectangle(image, (0, 0), (150, 65), (255, 0, 0), -1)  # background rectangle for on-screen text
 
        cv2.putText(image, "Vehicles in Area: " + str(currentcars), (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)
        cv2.putText(image, "Cars in Area: " + str(cars), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)
        cv2.putText(image, "Bikes in Area: " + str(bikes), (0, 45), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)
        cv2.putText(image, "Trucks in Area: 0", (0, 60), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)
